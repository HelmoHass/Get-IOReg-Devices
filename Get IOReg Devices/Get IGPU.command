...
def get_IGPU(IGPU):
    ...
    try:
        hid = "IGPU@" + IGPU.split("IGPU@")[1].split()[0]
    except:
        return None
    # Got our IGPU address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "IGPU@" in line:
            continue
        if not primed:
            # Has IGPU
            try:
                hdevice["name"] = "IGPU@" + line.split("IGPU@")[1].split()[0]
            except:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "IGPU" in line:
            IGPU.append(line)
    if not len(IGPU):
        print("IGPU not found!")
        exit(1)
    # Iterate through our IGPUs and get the info
    hlist = []
    for h in IGPU:
        d = get_IGPU(h)
        if d:
