...
def get_EH02(EH02):
    ...
    try:
        hid = "EH02@" + EH02.split("EH02@")[1].split()[0]
    except:
        return None
    # Got our EH02 address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "EH02@" in line:
            continue
        if not primed:
            # Has EH02
            try:
                hdevice["name"] = "EH02@" + line.split("EH02@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not EH02, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "EH02" in line:
            EH02.append(line)
    if not len(EH02):
        print("EH02 not found!")
        exit(1)
    # Iterate through our EH02s and get the info
    hlist = []
    for h in EH02:
        d = get_EH02(h)
        if d:
