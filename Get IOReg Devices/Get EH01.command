...
def get_EH01(EH01):
    ...
    try:
        hid = "EH01@" + EH01.split("EH01@")[1].split()[0]
    except:
        return None
    # Got our EH01 address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "EH01@" in line:
            continue
        if not primed:
            # Has EH01
            try:
                hdevice["name"] = "EH01@" + line.split("EH01@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not EH01, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "EH01" in line:
            EH01.append(line)
    if not len(EH01):
        print("EH01 not found!")
        exit(1)
    # Iterate through our EH01s and get the info
    hlist = []
    for h in EH01:
        d = get_EH01(h)
        if d:
