...
def get_SBUS(SBUS):
    ...
    try:
        hid = "SBUS@" + SBUS.split("SBUS@")[1].split()[0]
    except:
        return None
    # Got our SBUS address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "SBUS@" in line:
            continue
        if not primed:
            # Has SBUS
            try:
                hdevice["name"] = "SBUS@" + line.split("SBUS@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not SBUS, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "SBUS" in line:
            SBUS.append(line)
    if not len(SBUS):
        print("SBUS not found!")
        exit(1)
    # Iterate through our SBUSs and get the info
    hlist = []
    for h in SBUS:
        d = get_SBUS(h)
        if d:
