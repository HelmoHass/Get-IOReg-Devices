...
def get_SATA(SATA):
    ...
    try:
        hid = "SATA@" + SATA.split("SATA@")[1].split()[0]
    except:
        return None
    # Got our SATA address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "SATA@" in line:
            continue
        if not primed:
            # Has SATA
            try:
                hdevice["name"] = "SATA@" + line.split("SATA@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not SATA, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "SATA" in line:
            SATA.append(line)
    if not len(SATA):
        print("SATA not found!")
        exit(1)
    # Iterate through our SATAs and get the info
    hlist = []
    for h in SATA:
        d = get_SATA(h)
        if d:
