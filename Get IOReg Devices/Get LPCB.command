...
def get_LPCB(LPCB):
    ...
    try:
        hid = "LPCB@" + LPCB.split("LPCB@")[1].split()[0]
    except:
        return None
    # Got our LPCB address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "LPCB@" in line:
            continue
        if not primed:
            # Has LPCB
            try:
                hdevice["name"] = "LPCB@" + line.split("LPCB@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not LPCB, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "LPCB" in line:
            LPCB.append(line)
    if not len(LPCB):
        print("LPCB not found!")
        exit(1)
    # Iterate through our LPCBs and get the info
    hlist = []
    for h in LPCB:
        d = get_LPCB(h)
        if d:
