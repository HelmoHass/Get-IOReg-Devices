...
def get_HDAU(HDAU):
    ...
    try:
        hid = "HDAU@" + HDAU.split("HDAU@")[1].split()[0]
    except:
        return None
    # Got our HDAU address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "HDAU@" in line:
            continue
        if not primed:
            # Has HDAU
            try:
                hdevice["name"] = "HDAU@" + line.split("HDAU@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not HDAU, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "HDAU" in line:
            HDAU.append(line)
    if not len(HDAU):
        print("HDAU not found!")
        exit(1)
    # Iterate through our HDAUs and get the info
    hlist = []
    for h in HDAU:
        d = get_HDAU(h)
        if d:
