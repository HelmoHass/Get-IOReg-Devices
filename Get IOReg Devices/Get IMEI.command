...
def get_IMEI(IMEI):
    ...
    try:
        hid = "IMEI@" + IMEI.split("IMEI@")[1].split()[0]
    except:
        return None
    # Got our IMEI address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "IMEI@" in line:
            continue
        if not primed:
            # Has IMEI
            try:
                hdevice["name"] = "IMEI@" + line.split("IMEI@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not IMEI, not next device - must be info
        try:
...
def main():
    ...
    if not len(IMEI):
        ...
        exit(1)
    # Iterate through our IMEIs and get the info
    hlist = []
    for h in IMEI:
        d = get_IMEI(h)
        if d:
