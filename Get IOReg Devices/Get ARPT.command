...
def get_ARPT(ARPT):
    ...
    try:
        hid = "ARPT@" + ARPT.split("ARPT@")[1].split()[0]
    except:
        return None
    # Got our ARPT address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "ARPT@" in line:
            continue
        if not primed:
            # Has ARPT
            try:
                hdevice["name"] = "ARPT@" + line.split("ARPT@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not ARPT, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "ARPT" in line:
            ARPT.append(line)
    if not len(ARPT):
        print("ARPT not found!")
        exit(1)
    # Iterate through our ARPTs and get the info
    hlist = []
    for h in ARPT:
        d = get_ARPT(h)
        if d:
