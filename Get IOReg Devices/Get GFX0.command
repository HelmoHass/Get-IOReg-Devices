...
def get_PEGP(PEGP):
    ...
    try:
        hid = "PEGP@" + PEGP.split("PEGP@")[1].split()[0]
    except:
        return None
    # Got our PEGP address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "PEGP@" in line:
            continue
        if not primed:
            # Has PEGP
            try:
                hdevice["name"] = "PEGP@" + line.split("PEGP@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not PEGP, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "PEGP" in line:
            PEGP.append(line)
    if not len(PEGP):
        print("PEGP not found!")
        exit(1)
    # Iterate through our PEGPs and get the info
    hlist = []
    for h in PEGP:
        d = get_PEGP(h)
        if d:
