...
def get_MCHC(MCHC):
    ...
    try:
        hid = "MCHC@" + MCHC.split("MCHC@")[1].split()[0]
    except:
        return None
    # Got our MCHC address - get the full info
    ...
    for line in hd.split("\n"):
        if not primed and not "MCHC@" in line:
            continue
        if not primed:
            # Has MCHC
            try:
                hdevice["name"] = "MCHC@" + line.split("MCHC@")[1].split()[0]
            except:
        ...
        if "+-o" in line:
            ...
            primed = False
            continue
        # Primed, not MCHC, not next device - must be info
        try:
...
def main():
    ...
    for line in ioreg[0].split("\n"):
        if "MCHC" in line:
            MCHC.append(line)
    if not len(MCHC):
        print("MCHC not found!")
        exit(1)
    # Iterate through our MCHCs and get the info
    hlist = []
    for h in MCHC:
        d = get_MCHC(h)
        if d:
